...
class GetCard(GetMethods):
        ...
        for card_type in card_types:
            if card_type not in self.card_count:
                if stop_on_missing_card:
                    ...
                    raise RuntimeError('%r is not in the card_count; keys=%s' % (card_type, keys))
                out[card_type] = []
                continue
    ...
    def get_load_arrays(self, subcase_id, eid_map, node_ids, normals, nid_map=None):
        ...
        is_loads, is_temperatures, temperature_data, load_data = get_load_arrays(
            self, subcase_id, eid_map, node_ids, normals, nid_map=nid_map)
    ...
    def get_pressure_array(self, load_case_id, eids, stop_on_failure=True):
        ...
        return get_pressure_array(self, load_case_id, eids, stop_on_failure=stop_on_failure)
