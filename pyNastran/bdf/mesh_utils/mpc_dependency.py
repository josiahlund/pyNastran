...
def get_mpc_node_ids(model, mpc_id, consider_mpcadd=True, stop_on_failure=True):
    # type: (Any, int, bool, bool) -> List[List[int]]
    r"""
...
def get_mpc_node_ids_c1(model, mpc_id, consider_mpcadd=True, stop_on_failure=True):
    # type: (Any, int, bool, bool) -> Tuple[Dict[str, List[int]], Dict[str, List[int]]]
    r"""
    Get the MPC/MPCADD IDs.
...
def get_rigid_elements_with_node_ids(model, node_ids):
    """
...
def get_dependent_nid_to_components(model, mpc_id=None, stop_on_failure=True):
    """
...
def get_mpcs(model, mpc_id, consider_mpcadd=True, stop_on_failure=True):
    """
