...
def nastran_to_tecplot(model):
    ...
    if ntris and not nnot_tris and not nsolids:
        tecplot.tri_elements = np.array(tris, dtype='int32')
    elif nquads and not nnot_quads and not nsolids:
        tecplot.quad_elements = np.array(quads, dtype='int32')
    elif ntets and not nnot_tets and not nshells:
        tecplot.tet_elements = np.array(tets, dtype='int32')
    elif nhexas and not nnot_hexas and not nshells:
        tecplot.hexa_elements = np.array(hexas, dtype='int32')
    elif not nshells:
        ...
        tecplot.hexa_elements = np.array(elements)
    elif not nsolids:
...
def nastran_to_tecplot_filename(bdf_filename, tecplot_filename, log=None, debug=False):
    ...
    tecplot.results = np.array([], dtype='float32')
    return tecplot
